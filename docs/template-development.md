...
`.github/workflows/initialize-repository.yml` orchestrates the GitHub path. It uses `contents: write`, keeps checkout credentials from being persisted, invokes the helper, runs locked restore, format verification, Release build, tests, pack, package verification, commits the generated changes, and pushes to the selected default branch. The workflow should fail with actionable output when rulesets or branch protection prevent that one-time push; it must not weaken repository security settings automatically.
